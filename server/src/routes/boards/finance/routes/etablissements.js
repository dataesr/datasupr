...
router.get("/finance-universite/etablissements", async (req, res) => {
  try {
...
router.get(
  "/finance-universite/etablissements/:id/detail",
  async (req, res) => {
    try {
      const key = cacheKey(req.path, req.query);
      const hit = getCached(key);
...
router.get(
  "/finance-universite/etablissements/:id/overview",
  async (req, res) => {
    try {
      const key = cacheKey(req.path, req.query);
      const hit = getCached(key);
...
router.get(
  "/finance-universite/etablissements/:id/evolution",
  async (req, res) => {
    try {
      const key = cacheKey(req.path, req.query);
      const hit = getCached(key);
