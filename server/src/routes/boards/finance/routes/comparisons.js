...
router.get("/finance-universite/comparisons/compare", async (req, res) => {
  try {
...
router.get("/finance-universite/comparisons/advanced", async (req, res) => {
  try {
    const key = cacheKey(req.path, req.query);
    const hit = getCached(key);
    if (hit) return res.json(hit);
...
router.get("/finance-universite/comparaisons", async (req, res) => {
  try {
    const key = cacheKey(req.path, req.query);
    const hit = getCached(key);
    if (hit) return res.json(hit);
...
router.get("/finance-universite/comparisons/data", async (req, res) => {
  try {
    const key = cacheKey(req.path, req.query);
    const hit = getCached(key);
    if (hit) return res.json(hit);
