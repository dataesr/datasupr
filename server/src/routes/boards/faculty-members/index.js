  ...
  try {
      ...
      .aggregate([
        ...
        { $unwind: "$subject" },
        {
          $group: {
            ...
            subject_id: { $first: "$subject.id" }, // id arbitraire
          },
        },
        {
          $group: {
    ...
    const geos = await collection
      .aggregate([
        {
          $match: {
            niveau_geo: { $in: ["Région", "Académie"] }, // Inclut les deux
          },
        },
        {
          $group: {
        ...
        { $sort: { niveau_geo: 1, geo_nom: 1 } }, // Optionnel : pour avoir régions puis académies
      ])
      .toArray();
...
router.get("/faculty-members-geo-data/:geoId", async (req, res) => {
  try {
