...
async function run() {
  try {
    ...
    const structureIds = await personnelEnseignantCollection.distinct(
      "etablissement_id_paysage"
    );
    const catProIds = await personnelEnseignantCollection.distinct(
      "code_categorie_pers"
    );
    const disciplineIds = await personnelEnseignantCollection.distinct(
      "code_grande_discipline"
    );
    const cnuIds = await personnelEnseignantCollection.distinct(
      "code_groupe_cnu"
    );
    ...
    for (let i = 0; i < years.length; i++) {
      for (let j = 0; j < structureIds.length; j++) {
        const query = {
          etablissement_id_paysage: structureIds[j],
          rentree: years[i],
        };
        ...
        for (let k = 0; k < catProIds.length; k++) {
          const subData = data.filter(
            (item) => item.code_categorie_pers === catProIds[k]
          );

          const number_w = subData
            .filter((item) => item.sexe === "Féminin")
            .reduce((acc, item) => acc + item.effectif, 0);
          const number_m = subData
            .filter((item) => item.sexe === "Masculin")
            .reduce((acc, item) => acc + item.effectif, 0);
          const number_gender_u = subData
          ...
          for (let l = 0; l < cnuIds.length; l++) {
            const subDisciplineData = subData.filter(
              (item) => item.code_groupe_cnu === cnuIds[l]
            );

            const number_woman = subDisciplineData
              .filter((item) => item.sexe === "Féminin")
              .reduce((acc, item) => acc + item.effectif, 0);
            const number_man = subDisciplineData
              .filter((item) => item.sexe === "Masculin")
              .reduce((acc, item) => acc + item.effectif, 0);
            const number_gender_unknown = subDisciplineData
              .filter((item) => item.sexe === "Non renseigné")
              .reduce((acc, item) => acc + item.effectif, 0);
            const total = subDisciplineData.reduce(
        ...
        for (let m = 0; m < cnuIds.length; m++) {
          const subData = data.filter(
            (item) => item.code_groupe_cnu === cnuIds[m]
          );
          ...
          for (let n = 0; n < catProIds.length; n++) {
            const subDisciplineData = subData.filter(
              (item) => item.code_categorie_pers === catProIds[n]
            );
        ...
        for (let o = 0; o < disciplineIds.length; o++) {
          const subData = data.filter(
            (item) => item.code_grande_discipline === disciplineIds[o]
          );
          ...
          for (let p = 0; p < subCnuIds.length; p++) {
            const subDisciplineData = subData.filter(
              (item) => item.code_groupe_cnu === subCnuIds[p]
            );
        ...
        const number_woman = data
          .filter((item) => item.sexe === "Féminin")
          .reduce((acc, item) => acc + item.effectif, 0);
        const number_man = data
          .filter((item) => item.sexe === "Masculin")
          .reduce((acc, item) => acc + item.effectif, 0);
        const number_gender_unknown = data
          .filter((item) => item.sexe === "Non renseigné")
          .reduce((acc, item) => acc + item.effectif, 0);

        const number_under_35 = data
          .filter((item) => item.classe_age3 === "35 ans et moins")
          .reduce((acc, item) => acc + item.effectif, 0);
        const number_over35_under56 = data
          .filter((item) => item.classe_age3 === "36 à 55 ans")
          .reduce((acc, item) => acc + item.effectif, 0);
        const number_over56 = data
          .filter((item) => item.classe_age3 === "56 ans et plus")
          .reduce((acc, item) => acc + item.effectif, 0);
        const number_age_unknown = data
          .filter((item) => item.classe_age3 === "Non précisé")
          .reduce((acc, item) => acc + item.effectif, 0);

        const number_full_time = data
          .filter((item) => item.quotite === "Temps plein")
          .reduce((acc, item) => acc + item.effectif, 0);
        const number_part_time = data
          .filter((item) => item.quotite === "Temps partiel")
          .reduce((acc, item) => acc + item.effectif, 0);
        const number_time_unknown = data
          .filter((item) => item.quotite === "Non renseigné")
          .reduce((acc, item) => acc + item.effectif, 0);
