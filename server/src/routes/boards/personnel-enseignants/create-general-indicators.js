...
async function run() {
    try {
      ...
      const calculsCollection  = database.collection("teaching-staff-general-indicators");

      await calculsCollection.deleteMany({});

      const years = await personnelEnseignantCollection.distinct("rentree");
      // const years = [2022,2023];
      const geoIds = await personnelEnseignantCollection.distinct("etablissement_code_region");
      const acaIds = await personnelEnseignantCollection.distinct("etablissement_code_academie");
      const disciplineIds = await personnelEnseignantCollection.distinct("code_grande_discipline");
      const assimilationIds = await personnelEnseignantCollection.distinct("code_categorie_assimil");

      // Loop for regional
      for (let i = 0; i < years.length; i++) {
          for (let j = 0; j < geoIds.length; j++) {
            const query = { etablissement_code_region: geoIds[j], rentree: years[i] };
            const data = await personnelEnseignantCollection.find(query).toArray();

            const effectifSexeF = data.filter((item) => item.sexe === 'Féminin').reduce((acc, item) => acc + item.effectif, 0);
            const effectifSexeM = data.filter((item) => item.sexe === 'Masculin').reduce((acc, item) => acc + item.effectif, 0);
            const effectifSexeNR = data.filter((item) => item.sexe === 'Non renseigné').reduce((acc, item) => acc + item.effectif, 0);
            ...
            for (let k = 0; k < subDisciplineIds.length; k++) {
              const subData = data.filter((item) => item.code_grande_discipline ===  subDisciplineIds[k]);
            ...
            for (let k = 0; k < subAssimilationIds.length; k++) {
              const subData = data.filter((item) => item.code_categorie_assimil ===  subAssimilationIds[k]);
      ...
      for (let i = 0; i < years.length; i++) {
          for (let j = 0; j < acaIds.length; j++) {
            const query = { etablissement_code_academie: acaIds[j], rentree: years[i] };
            const data = await personnelEnseignantCollection.find(query).toArray();
