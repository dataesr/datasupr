...
router
  .route(routesPrefix + "/top10-countries-by-type-of-beneficiaries")
  .get(async (req, res) => {
    ...
    try {
      ...
      const targetCountryIndex = resultData.findIndex(
        (item) => item.country_code === targetCountry
      );
      ...
      if (targetCountryIndex >= 10) {
        ...
        if (targetCountryData.length > 0) {
          ...
          console.warn(
            `Aucune donnée trouvée pour le pays ${targetCountry} avec les filtres appliqués`
          );
        }
      }
...
router
  .route(routesPrefix + "/top10-countries-by-type-of-beneficiaries_indexes")
  .get(async (req, res) => {
    try {
      await recreateIndex(
      ...
      res.status(201).json({
        message:
          "Index couvert pour top10-countries-by-type-of-beneficiaries créé avec succès",
...
router
  .route(routesPrefix + "/type-beneficiaries-evolution")
  .get(async (req, res) => {
    console.log(
      "Fetching type beneficiaries evolution with filters:",
      req.query
    );
    ...
    if (!req.query.cordis_type_entity_code) {
      return res
        .status(400)
        .json({ error: "Le code type d'entité CORDIS est requis" });
    }
    ...
    try {
      ...
      const targetCountryIndex = top10Countries.findIndex(
        (item) => item.country_code === targetCountry
      );
      ...
      if (targetCountryIndex === -1) {
        ...
        if (targetCountryData.length > 0) {
          ...
          return res.status(404).json({
            error: "Aucune donnée trouvée pour ce pays et ce type d'entité",
          });
        }
      }
          ...
          .aggregate([
            {
              $match: {
                ...
                ...(req.query.destinations && {
                  destination_code: { $in: req.query.destinations.split(",") },
                }),
              },
            },
            {
      ...
      const completeData = evolutionData.map((country) => {
        ...
        const completeEvolution = yearsList.map((year) => {
          const existingData = country.evolution.find(
            (e) =>
              e.year === year ||
...
router
  .route(routesPrefix + "/type-beneficiaries-evolution_indexes")
  .get(async (req, res) => {
    try {
      await recreateIndex(
        db.collection("european-projects_projects-entities_staging"),
      ...
      res.status(201).json({
        message:
          "Index couvert pour type-beneficiaries-evolution créé avec succès",
