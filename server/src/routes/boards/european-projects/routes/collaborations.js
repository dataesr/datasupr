      ...
      .aggregate([
        {
          $match: {
            ...(entityName && {
              entities_name: { $regex: entityName, $options: "i" },
            }),
...
router
  .route(routesPrefix + "/get-collaborations_indexes")
  .get(async (req, res) => {
    try {
      await recreateIndex(
...
router
  .route(routesPrefix + "/get-collaborations-by-country")
  .get(async (req, res) => {
...
router
  .route(routesPrefix + "/get-collaborations-by-country_indexes")
  .get(async (req, res) => {
    try {
      await recreateIndex(
        db.collection("european-projects_collaborations_staging"),
...
router
  .route(routesPrefix + "/get-collaborations-by-entity")
  .get(async (req, res) => {
    ...
    if (!entity_id) {
      return res
        .status(400)
        .json({ error: "L'identifiant de l'entité est requis" });
    }
                ...
                },
                {
                  $addFields: {
                    total_projects: {
                      $size: {
                        $ifNull: [
                          { $arrayElemAt: ["$entity_stats.total_projects", 0] },
                          [],
                        ],
                      },
                    },
...
router
  .route(routesPrefix + "/get-collaborations-by-entity_indexes")
  .get(async (req, res) => {
    try {
      await Promise.all([
        recreateIndex(
