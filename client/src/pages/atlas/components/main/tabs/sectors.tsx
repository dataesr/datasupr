        ...
        dataSimilarSorted?.length > 0 && (
          <Row className="fr-mt-5w">
            <Col>
              ...
              <Text>
                Les territoires similaires sont ceux qui ont une répartition du nombre d'étudiants inscrits dans le secteur public et privé proche du territoire sélectionné.
                ...
                <ul>
                  {
                    dataSimilarSorted?.slice(0, 6).filter((el: SimilarData) => el.geo_id !== searchParams.get('geo_id')).map((el: SimilarData) => (
                      <li key={el.geo_id}>
                        {el.geo_nom} (Secteur public: <strong>{el.pctPU.toFixed(2)}%</strong> - Secteur privé: <strong>{el.pctPR.toFixed(2)}%</strong>)
                        <Button
                          size="sm"
                          variant="text"
                          className="fr-ml-1w"
