...
import { getColorFromFunder, getGeneralOptions } from "../../../../utils.ts";
import CountiesSelector from "../../components/countiesSelector.tsx";

const { VITE_APP_SERVER_URL } = import.meta.env;


export default function ParticipationsOverTimeByCounty() {
  const [selectedCounty, setSelectedCounty] = useState<string>("Bretagne");
    ...
    query: {
      bool: {
        filter: [
          ...
          {
            term: {
              "address.region.keyword": selectedCounty,
            },
          },
        ]
      }
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['fundings-participations-over-time-by-county', selectedCounty],
    queryFn: () =>
  ...
  const config = {
    id: "participationsOverTimeByCounty",
    integrationURL: "/integration?chart_id=participationsOverTimeByCounty",
    title: `Nombre de projets pour la région ${selectedCounty} par financeur sur la période ${years[0]}-${years[years.length - 1]}, par année de début du projet`,
  };
