...
export default function PatentsCoApplicants({ name }: { name: string | undefined }) {
  ...
  const filteredData = (data?.aggregations?.by_co_applicants?.buckets ?? []).filter((item) => item.key !== name);
  const categories = filteredData.map((bucket) => bucket?.key);
