...
export const createEvolutionRessourcesPropresChartOptions = (
  data: EvolutionData[],
  ...
  const yAxisConfig = {
    ...
    labels: {
      ...
      formatter: function (this: any) {
        ...
        const formats = selectedMetrics.map((key) => metricsConfig[key].format);
        const hasPercent = formats.includes("percent");
        const hasEuro = formats.includes("euro");

        if (hasPercent && !hasEuro) {
          return `${value.toFixed(1)}%`;
        }
        if (hasEuro && !hasPercent) {
