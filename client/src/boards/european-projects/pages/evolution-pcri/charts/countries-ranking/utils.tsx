...
export function readingKey(data, isLoading) {
  ...
  // Calculer le classement pour chaque framework
  ...
  const fr = (
    <>
      <strong>{countryName}</strong> affiche la plus forte progression entre {firstFramework} et {lastFramework}, passant de la{" "}
      <strong>{rankData.first}ème</strong> à la <strong>{rankData.last}ème</strong> place, soit un gain de <strong>{rankData.change}</strong> position
  ...
  const en = (
    <>
      <strong>{countryName}</strong> shows the strongest progression between {firstFramework} and {lastFramework}, moving from{" "}
      <strong>{rankData.first}th</strong> to <strong>{rankData.last}th</strong> place, a gain of <strong>{rankData.change}</strong> position
