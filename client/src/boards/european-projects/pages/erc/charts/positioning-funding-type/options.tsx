...
interface OptionsParams {
  processedData: ProcessedPositioningByFundingTypeData;
  currentLang?: string;
}
...
export default function Options({ processedData, currentLang = "fr" }: OptionsParams): Highcharts.Options | null {
  ...
  const fundingTypeLabel =
    FUNDING_TYPES.find((f) => f.code === processedData.fundingType)?.label[currentLang as "fr" | "en"] ?? processedData.fundingType;
  ...
  const titleText =
    processedData.metric === "projects"
      ? `${getI18nLabel(i18n, "chart-title-projects", currentLang)} – ${fundingTypeLabel}`
      : `${getI18nLabel(i18n, "chart-title-funding", currentLang)} – ${fundingTypeLabel}`;

  const colors = getChartColors(processedData.fundingType);
