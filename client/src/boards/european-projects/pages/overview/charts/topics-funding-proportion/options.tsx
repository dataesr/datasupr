    ...
    series: [
      {
        ...
        groupPadding: 0,
        data: data.data.filter((item) => item.stage === "evaluated").map((item) => [item.thema_name_fr, item.proportion]),
        dataLabels: {
          formatter: function () {
      ...
      },
      {
        ...
        groupPadding: 0,
        data: data.data.filter((item) => item.stage === "successful").map((item) => [item.thema_name_fr, item.proportion]),
        dataLabels: {
          formatter: function () {
            return formatToPercent(this.y ?? 0);
          },
