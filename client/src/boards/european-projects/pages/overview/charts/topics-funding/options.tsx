    ...
    series: [
      {
        ...
        groupPadding: 0,
        data: data.data.filter((item) => item.stage === "evaluated").map((item) => [item.thema_name_fr, item.total_fund_eur]),
        dataLabels: {
          formatter: function () {
      ...
      },
      {
        ...
        groupPadding: 0,
        data: data.data.filter((item) => item.stage === "successful").map((item) => [item.thema_name_fr, item.total_fund_eur]),
        dataLabels: {
          formatter: function () {
            return formatToMillions(this.y ?? 0);
          },
